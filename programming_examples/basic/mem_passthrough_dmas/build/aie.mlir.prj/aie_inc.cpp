...
const int in_cons_buff_0_offset = 0;
...
const int in_cons_buff_1_offset = 4096;
int32_t mlir_aie_read_buffer_in_cons_buff_1(aie_libxaie_ctx_t* ctx, int index) {
u32 value; auto rc = XAie_DataMemRdWord(&(ctx->DevInst), XAie_TileLoc(0,1), in_cons_buff_1_offset + (index*4), &value);
  return value;
}
int mlir_aie_write_buffer_in_cons_buff_1(aie_libxaie_ctx_t* ctx, int index, int32_t value) {
  int32_t int_value = value;
AieRC rc =    XAie_DataMemWrWord(&(ctx->DevInst), XAie_TileLoc(0,1), in_cons_buff_1_offset + (index*4), int_value);
return rc;
}
